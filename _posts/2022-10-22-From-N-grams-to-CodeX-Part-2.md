...
Observe that the LSB bit of the integer indices (recall that, in little-endian, the LSB is the **rightmost** bit, and the MSB is the **leftmost** bit) changes between each consecutive index (0,1,2,3,4 -> 000, 001, 010, 011, 100 observe how the LSB changes frequently), and the MSB bit changes rarely. Intuitively, you can think of these bits as representing different frequencies (the LSB bit has a high frequency, and the MSB bit has a low frequency). Please read [here](https://towardsdatascience.com/master-positional-encoding-part-i-63c05d90a0c3) for more on this intuition. How can we use this intuition to make the binary vectors into continuous representations of the input position?
